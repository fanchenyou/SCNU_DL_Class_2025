...
def main(args):
    ...
    # if you have a MAC M2/3 chip, turn this on
    ...
    # read in QA dataset
    # split all_dataset randomly to 6:2:2 for train, val , test set
    if not os.path.isfile('saves/data.pkl'):
        data_dict = []
        with open('data/train_proto_qa.csv', newline='', encoding='utf-8') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                data_dict.append(row)
        with open('saves/data.pkl','wb') as f:
            pickle.dump(data_dict, f)
    else:
        with open('saves/data.pkl','rb') as f:
            data_dict = pickle.load(f)
            

    if not os.path.isfile('saves/vocab.pkl'):
        vocab_by_id, vocab_by_token = build_vocab(data_dict)
        with open('saves/vocab.pkl','wb') as f:
            pickle.dump([vocab_by_id, vocab_by_token], f)
    else:
        with open('saves/vocab.pkl','rb') as f:
            vocab_by_id, vocab_by_token = pickle.load(f)

    print(vocab_by_token)
    vocab_size = len(vocab_by_token)
    print('dictionary length', vocab_size)

    train_set = qa_dataset(data_dict, vocab_by_id, vocab_by_token)
    train_loader = DataLoader(train_set, batch_size=1, shuffle=True, drop_last=True)
    # validate_loader = DataLoader(qa_dataset(val_set,vocab_by_id, vocab_by_token), batch_size=1, shuffle=False)
    # test_loader = DataLoader(qa_dataset(test_set,vocab_by_id, vocab_by_token), batch_size=1, shuffle=False)
