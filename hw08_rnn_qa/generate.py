...
def generate(model, dset, query_str='A', max_predict_len=100, device=None):
    ...
    query_input = dset.get_tensor(query_str).to(device)

    # TODO: Explain, why we need pass [BEG] token into generate
    query_answer_init_token = dset.get_beg_tensor().to(device)
    # TODO: Explain function of encoding stage
    predicted = model.generate(query_input, query_answer_init_token,
                               dset.pos_end_token_id, max_predict_len, device=device)

    # if last token is [END], remove it and not print
    if predicted[-1] == dset.pos_end_token_id:
        predicted = predicted[:-1]

    # convert token index back into word
    word_list = [dset.get_word(ind) for ind in predicted]
...
if __name__ == '__main__':
    ...
    with open('saves/data.pkl','rb') as f:
        train_set = pickle.load(f)
    dset = qa_dataset(train_set, vocab_by_id, vocab_by_token)
            
    response = generate(model, dset, args.query_str, args.predict_len, device)
