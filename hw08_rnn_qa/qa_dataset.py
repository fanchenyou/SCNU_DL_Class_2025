...
def build_vocab(data_dict):
    ...
    for row in data_dict:
        ...
        # use sp to tokenizer
        #words = sp.encode_as_pieces(sent)
        # print(words0)
        # print(words)
        # print()
    ...
    # keep words over 3 times
    word_counter = {k:v for k,v in word_counter.items() if v >= 3}
    
    for ix, word_key in enumerate(sorted(list(word_counter))):
        vocab_by_id[ix] = word_key
        vocab_by_token[word_key] = ix

    begin_token = '<BEG>'
    end_token = '<END>'
    unknown_token = '<UNK>'
    special_keys = [begin_token, end_token, unknown_token]

    tmp_len = len(vocab_by_id)
    for ix, skey in enumerate(special_keys):
        vocab_by_id[tmp_len + ix] = skey
        vocab_by_token[skey] = tmp_len + ix
        
    return vocab_by_id, vocab_by_token


class qa_dataset(data.Dataset):
    def __init__(self, data_dict, vocab_by_id, vocab_by_token):
        ...
        self.vocab_by_id = vocab_by_id
        self.vocab_by_token = vocab_by_token
